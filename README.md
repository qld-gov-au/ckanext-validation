...
[![CI Actions Status](https://github.com/ckan/ckanext-validation/actions/workflows/Tests/badge.svg)](https://github.com/ckan/ckanext-validation/actions)
[![Coverage Status](https://coveralls.io/repos/github/ckan/ckanext-validation/badge.svg?branch=master)](https://coveralls.io/github/ckan/ckanext-validation?branch=master)
...
If using ckanext-scheming, the `validation` plugins should be loaded **before** the `scheming_datasets` one:
...
Use the following configuration options to choose the [operation modes](#operation-modes):
    ...
    ckanext.validation.run_on_create_async = True|False (Defaults to False)    
    ckanext.validation.run_on_update_async = True|False (Defaults to False)
...
used by default when running the validation:
    ...
    ckanext.validation.default_validation_options={
        "skip_errors": ["blank-row", "duplicate-row"],
        }

see: https://github.com/frictionlessdata/frictionless-py/blob/main/frictionless/errors/row.py
...
The plugin also provides the `IPipeValidation` interface so other plugins can receive 
the dictized validation reports in a Data Pipeline way. This would allow plugins to 
perform actions once a validation job is finished.
...
`validation run` command. If no parameters are provided it will start a 
...
success, failure or error. Additionally they will create a CSV report. 
...
`validation report-full` will add a row on the output CSV for each error 
